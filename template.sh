...
LOCALE=""
EDITOR_CHOICE=""
btrfs_mnt_options="compress=zstd,noatime"
...
get_btrfs_layout() {
  ...
  if yes_no_prompt "Use default Btrfs mount options (compress=zstd,noatime)?"; then
    ...
    if ! read -rp "Enter Btrfs mount options (e.g. compress=lzo,noatime): " btrfs_mnt_options; then
      fatal "Input aborted"
    fi
    success "Btrfs mount options set to: $btrfs_mnt_options"
  fi
}
...
mount_filesystems() {
  ...
  if [[ "$FILESYSTEM_TYPE" == "btrfs" ]]; then
    ...
    mount -o subvol=@,$btrfs_mnt_options "$root_partition" "$MOUNT_POINT"

    mkdir -p "$MOUNT_POINT/home" "$MOUNT_POINT/var/log" "$MOUNT_POINT/var/cache/pacman/pkg"
    mount -o subvol=@home,$btrfs_mnt_options "$root_partition" "$MOUNT_POINT/home"
    mount -o subvol=@log,$btrfs_mnt_options "$root_partition" "$MOUNT_POINT/var/log"
    mount -o subvol=@pkg,$btrfs_mnt_options "$root_partition" "$MOUNT_POINT/var/cache/pacman/pkg"
